...
import * as ComponentsJson_2 from '../../../../assets/JSON_Files/component_picker_Final_JSON_2.json';
...
export class SecondJsonComponentComponent implements OnInit {
  ...
  @ViewChild('speedChangeDialog_next') speedChangeDialog = {} as TemplateRef<string>;
  ...
  public initializeSelectedCompnents() {
    ...
    this.currentSelectedValue = "Belt or Chain Drive";
    this.stringJson1 = JSON.stringify(ComponentsJson_2);
    ...
    let gearboxSelected = this.checkIfGearBoxAlreadySelected();
    if (gearboxSelected) {
      this.components.forEach((item: any, index: any) => {
  ...
  checkIfGearBoxAlreadySelected() {
    let allConfiguration = this.sharedService.getAllInsertedConfigurations();
    let gearboxSelected: boolean = false;
    if (allConfiguration !== null && allConfiguration.length > 0) {
      allConfiguration.forEach(element => {
        if (element.includes("component:Gearbox")) {
          gearboxSelected = true;
        }
      });
    }
    return gearboxSelected;
  }
  ngOnInit(): void {
  ...
  constructFinalKey(currentState: string, currentEAKey: string, key: string, currentSelectedValue: any) {
    ...
    switch (currentState) {
      ...
      /* start added for 2nd json component */
      case "GCPL2-S1": {
        if (this.currentEAKey === 'cpl2' && currentSelectedValue === "Belt Drive") {
          constructedKey = "cpl2=='Belt Drive'";
        }
        else {
          constructedKey = 'exit';
        }
      ...
      /*end added for 2nd json component */
      default: {
        constructedKey = key;
        break;
      }
